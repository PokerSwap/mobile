...
export default TournamentSort = (props) => {
  ...
  const checkSort = (an_order) => {  
    !(an_order=='asc' && props.order=='asc' )?
      !(an_order=='desc' && props.order=='desc') ?
        startSort(props.order, props.searchInput)
        :
        null
      :
      null
  ...
  const startSort = async (an_order2, searchInput) => {
    ...
    console.log('order after', an_order2)
  ...
  const getLocation = () => {
    ...
    let myCoordinates = Geolocation.getCurrentPosition(info => {
      ...
      console.log('long',a_longitude)
      console.log('lat',a_latitude)
  ...
  return(
    <View style={{flexDirection:'row', justifyContent:'space-around'}}>
      <Segment>
        <Button first active={asc} onPress={()=> checkSort('asc')}>
          <Text>Asc.</Text>
        </Button>
        <Button last active={desc} onPress={()=> checkSort('desc')}>
          <Text>Desc.</Text>
        </Button>
      </Segment>
      <Segment >
        <Button first active={zipcode} onPress={()=> console.log('lol')}>
          <Text>Zip Code</Text>
        </Button>
        <Button last active={location} onPress={()=> getLocation(props.limit)}>
